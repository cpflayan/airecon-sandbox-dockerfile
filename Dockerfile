...
RUN apt-get update && \
    ...
    fonts-freefont-ttf fonts-dejavu-core ttf-bitstream-vera libnss3-tools libpcap-dev \
    # Networking & Pivoting
    net-tools dnsutils whois iproute2 iputils-ping netcat-traditional socat \
...
RUN go install -v github.com/projectdiscovery/httpx/cmd/httpx@latest && \
    ...
    go install -v github.com/hakluke/hakoriginfinder@latest && \
...
RUN cp -n /home/pentester/go/bin/* /usr/local/bin/ 2>/dev/null || true && \
    ...
    ln -sf /usr/local/bin/stratus /usr/local/bin/stratus-red-team 2>/dev/null || true && \
    # nomore403 Go install produces binary 'nomore403'; alias to catalog name 'nowaf'
    ln -sf /usr/local/bin/nomore403 /usr/local/bin/nowaf 2>/dev/null || true && \
    # ligolo-ng Go install cmd/proxy produces binary 'proxy'; alias to catalog name
    ln -sf /usr/local/bin/proxy /usr/local/bin/ligolo-ng 2>/dev/null || true && \
    # VhostFinder Go install produces binary 'VhostFinder' (capital V); alias lowercase
    ln -sf /usr/local/bin/VhostFinder /usr/local/bin/vhostfinder 2>/dev/null || true

# ── Rust tools (pre-compiled binaries, optional — build continues if download fails) ──
# ── RustScan (fast port scanner) ──
RUN (RUSTSCAN_VER=$(curl -s https://api.github.com/repos/RustScan/RustScan/releases/latest | grep -oP '"tag_name":\s*"\K[^"]+' | head -1) && \
    RUSTSCAN_VER=${RUSTSCAN_VER:-2.3.0} && \
    curl -fsL "https://github.com/RustScan/RustScan/releases/download/${RUSTSCAN_VER}/rustscan_${RUSTSCAN_VER}_amd64.deb" -o /tmp/rustscan.deb && \
    dpkg -i /tmp/rustscan.deb && rm -f /tmp/rustscan.deb) || \
    echo "[WARN] rustscan installation skipped"

RUN (curl -fsL "https://github.com/Sh1Yo/x8/releases/download/v4.3.0/x86_64-linux-x8.gz" -o /tmp/x8.gz && \
    gunzip /tmp/x8.gz && chmod +x /tmp/x8 && mv /tmp/x8 /usr/local/bin/x8) || \
    echo "[WARN] x8 installation skipped (release asset not found)"
RUN (curl -fsL "https://gitlab.com/api/v4/projects/33695681/packages/generic/nrich/0.4.2/nrich-linux-x86_64" -o nrich && \
    chmod +x nrich && mv nrich /usr/local/bin/ && rm -f nrich) || \
    echo "[WARN] nrich installation skipped (release asset not found)"

# ── Kiterunner (pre-compiled binary) ──
RUN wget -qO /tmp/kiterunner.tar.gz https://github.com/assetnote/kiterunner/releases/download/v1.0.2/kiterunner_1.0.2_linux_amd64.tar.gz && \
    tar -xzf /tmp/kiterunner.tar.gz -C /tmp && \
    mv /tmp/kr /usr/local/bin/kr && \
    chmod +x /usr/local/bin/kr && \
    ln -s /usr/local/bin/kr /usr/local/bin/kiterunner && \
...
RUN pipx install arjun && \
    ...
    pipx install git+https://github.com/doyensec/GQLSpection.git && \
    pipx install git+https://github.com/xnl-h4ck3r/xnLinkFinder.git && \
    pipx install git+https://github.com/xnl-h4ck3r/xnldorker.git && \
    pipx install git+https://github.com/MandConsultingGroup/porch-pirate.git && \
    pipx install git+https://github.com/roys/cewler.git && \
    pipx install wfuzz && \
    pipx install whatwaf && \
    pipx install git+https://github.com/s0md3v/XSStrike.git 2>/dev/null || true && \
    pipx install git+https://github.com/vladko312/SSTImap.git 2>/dev/null || true && \
    pipx install git+https://github.com/defparam/smuggler.git 2>/dev/null || true && \
    pipx install schemathesis 2>/dev/null || true
...
RUN find /home/pentester/.local/bin -maxdepth 1 \( -type f -o -type l \) -exec ln -sf {} /usr/local/bin/ \; 2>/dev/null || true && \
    ...
    # scoutsuite pipx installs binary 'scout'; alias to catalog name
    ln -sf /usr/local/bin/scout /usr/local/bin/scoutsuite 2>/dev/null || true
USER pentester
...
RUN git clone https://github.com/aravind0x7/JS-Snooper.git 2>/dev/null || true && \
    ...
    git clone --depth=1 https://github.com/epinna/tplmap.git 2>/dev/null || true && \
    git clone --depth=1 https://github.com/aquasecurity/cloudsploit.git 2>/dev/null || true && \
    ...
    git clone https://github.com/Tuhinshubhra/CMSeeK.git 2>/dev/null || true && \
    git clone https://github.com/UndeadSec/SwaggerSpy.git 2>/dev/null || true && \
    git clone https://github.com/MattKeeley/Spoofy.git 2>/dev/null || true && \
    git clone https://github.com/Arcanum-Sec/msftrecon.git 2>/dev/null || true && \
    git clone --depth=1 https://github.com/vincentcox/bypass-firewalls-by-DNS-history.git 2>/dev/null || true && \
    git clone --depth=1 https://github.com/NullArray/DorkNet.git 2>/dev/null || true && \
    true && \
    (cd cloudsploit && npm install 2>/dev/null && \
    ln -sf /home/pentester/tools/cloudsploit/index.js /home/pentester/.local/bin/cloudsploit 2>/dev/null) || true
...
RUN wget -qO /usr/local/bin/linpeas.sh https://github.com/peass-ng/PEASS-ng/releases/latest/download/linpeas.sh && \
    ...
    ln -sf /usr/local/bin/linpeas.sh /usr/local/bin/linpeas && \
